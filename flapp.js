...
const buttonData = {
  ...
  "mozarella": { img: "pictos/fromPicto.png", label: "mozzarella" },
  "mozarella": { img: "pictos/fromPicto.png", label: "mozzarella" },
...
// Supposons que votre base soit dans window.data ou un tableau
// const data = Object.values(window.data); // Si c'est un objet
// addPizzaAddons(data);

// Ou directement sur window.data si c'est déjà un tableau
// addPizzaAddons(window.data);

// ============================================
// VERSION ALTERNATIVE : TRAITEMENT EN PLACE
// ============================================
...
class Memo {
  ...
  const pizzas = this.orders.filter(o =>
    ...
    !(o.name || "").toLowerCase().includes("margarita")     // (double orthographe)
  );
...
o.totalTTC = Number(totalTTC.toFixed(2));
  ...
  header.innerHTML = `
    ...
    <strong>Mémos</strong>
    <span class="memo-count">${this.orders.length} ligne${this.orders.length > 1 ? "s" : ""}</span>
  `;
  section.appendChild(header);
